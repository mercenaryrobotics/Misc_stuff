...
$M3_5Opening            = 3.7;
$M3_5CaptureNutDiameter = 6.85;
//Parameters for small bearing (686ZZ)
$RevBearingDiameter     = 13 + .2;
$RevBearingDepth        = 5  + .2;
$RevBearingBlockWidth   = 25;
$RevBearingBlockHeight  = 25;
$RevHexDiameter         = 5.9;
$RevPillowBlockDepth    = 8;
//Parameters for skate bearing (608ZZ)
$RevBearingDiameter     = 22 + .2;
$RevBearingDepth        = 7  + .2;
$RevBearingBlockWidth   = 25;
$RevBearingBlockHeight  = 27;
$RevHexDiameter         = 5.9;
$RevPillowBlockDepth    = 10;
...
//RevBearingBlock();
//RevOmniSprocketMount();
RevBearingPillow();
//RevOmniSpacer($MainHubDepth = 7);
//RevOmniSpacer($MainHubDepth = 2.5);
//RevOmniSpacer($MainHubDepth = 3.5);
//RevRoundSpacer($MainHubDepth = 2.5);
//RevOmniSetSpacer();
//RevHexSkateInsert();
//RevHexSkateInsertSet();

module RevHexSkateInsertSet()
{
  for (x =[0:4])
    for (y =[0:4])
...
module RevHexSkateInsert()
{
  difference()
  {
    cylinder(d1 = 8.05, d2 = 8.0, h = 7, $fn = 60, center = true);
    //Shaft
    cylinder(d = $RevHexDiameter, h = 30, $fn = 6, center = true);
  }
}

module RevOmniSetSpacer()
{
  $BearingBlockWidth        = $RevBearingBlockWidth;
  $BearingBlockHeight       = $RevBearingBlockHeight;
  $BearingBlockAlignerDepth = 1;
  $BearingDiameter          = $RevBearingDiameter;
  $BearingDepth             = $RevBearingDepth;
  $MountBoltholeDiameter    = 3.8;
  $MountBoltDepth           = 3.2;
  $MountChannelLength       = 15;
  $MountBoltWidth           = $M3_5Opening;
  $SetSpacerHeight          = 44;
  
  difference()
  {
    //Base block
    translate([-$BearingBlockWidth / 2, -$BearingBlockHeight / 2, 0])
      cube([$BearingBlockWidth, $BearingBlockHeight, $SetSpacerHeight]);
    
    //Mounting holes
    translate([8, 0, -0.01])
      cylinder(d = $MountBoltholeDiameter, h = $SetSpacerHeight + 0.02, $fn = 40);
    translate([-8, 0, -0.01])
      cylinder(d = $MountBoltholeDiameter, h = $SetSpacerHeight + 0.02, $fn = 40);
    translate([0, 8, -0.01])
      cylinder(d = $MountBoltholeDiameter, h = $SetSpacerHeight + 0.02, $fn = 40);
    ...
    //Catch bolt openings, bottom
    for (a =[0:3])
      rotate(90 * a, [0, 0, 1])
        translate([0, 8, 0 + 5])
          CaptureNut($CaptureNutDiameter = $M3_5CaptureNutDiameter + 0.2, $CaptureNutHeight   = 3.1, $Length = 15, $BoreDiameter = $MountBoltWidth, $BoreLength = 20, $BoreOffset = 6);//M3 nut      
    //Catch bolt openings, top
    for (a =[0:3])
      rotate(90 * a, [0, 0, 1])
        translate([0, 8, $SetSpacerHeight - 3.1 - 5])
          CaptureNut($CaptureNutDiameter = $M3_5CaptureNutDiameter + 0.2, $CaptureNutHeight   = 3.1, $Length = 15, $BoreDiameter = $MountBoltWidth, $BoreLength = 20, $BoreOffset = 6);//M3 nut      
  }
  //Channel insert aligners
  translate([0, 0, -2])
    cylinder(d = 8, h = 2, $fn = 60);
  translate([0, 0, $SetSpacerHeight])
    cylinder(d = 8, h = 2, $fn = 60);
}

module RevRoundSpacer()
{
  difference()
  {
    //Main wheel attach
    cylinder(d = 18, h = $MainHubDepth, $fn = 50);
    //Shaft
    cylinder(d = $RevHexDiameter, h = 30, $fn = 6);
    //Omni hub openings
    translate([0, 0, -0.01])
      cylinder(d = 9, h = 2.55, $fn = 40);
    translate([0, 0, $MainHubDepth + 0.31])
      cylinder(d = 9, h = 2.5, $fn = 40);
  }
}

module RevOmniSpacer()
{
  difference()
  {
    //Main wheel attach
    cylinder(d = 43, h = $MainHubDepth, $fn = 6);
    //Shaft
    cylinder(d = $RevHexDiameter, h = 30, $fn = 6);
    //Omni hub openings
    translate([0, 0, -0.01])
    ...
    for (a =[0:2])
    {
      rotate(120 * a, [0, 0, 1])
      translate([0, -16, -0.01])
      cylinder(d = $M3_5Opening, h = 20, $fn = 40);
    }
  }
}

module RevBearingPillow()
{
  difference()
  {
    union()
    {
      //Main block
      translate([-30, -$RevBearingBlockHeight / 2, 0])
        cube([60, $RevBearingBlockHeight, $RevPillowBlockDepth]);
      //Alignment pillars
      translate([16, 0, $RevPillowBlockDepth])
        cylinder(d = 7.7, h = 2, $fn = 40);
      translate([-16, 0, $RevPillowBlockDepth])
        cylinder(d = 7.7, h = 2, $fn = 40);
    }
    //Shaft opening
    cylinder(d = 7.6, h = 10, $fn = 40);
    //Bearing seat
    translate([0, 0, $RevPillowBlockDepth - $RevBearingDepth + 0.01])
      cylinder(d = $RevBearingDiameter, h = $RevBearingDepth, $fn = 40);
    //Mounting holes
    translate([-16, 0, 0])
      for (a =[1:7])
        rotate(45 * a, [0, 0, 1])
...
module CaptureNut()
{
    ...
    {
      rotate(30, [0, 0, 1])
       cylinder(d = $CaptureNutDiameter, h = $CaptureNutHeight, $fn = 6);
      translate([0, $Length, 0])
        rotate(30, [0, 0, 1])
         cylinder(d = $CaptureNutDiameter, h = $CaptureNutHeight, $fn = 6);
    }
    translate([0, 0, -$BoreOffset])
      cylinder(d = $BoreDiameter, h = $BoreLength, $fn = 50);
}

module RevOmniSprocketMount()
{
  $MainHubDepth = 7;
  ...
  difference()
  {
    ...
    translate([0, 0, 4])
    {
      translate([0, 7, 0])
        rotate(-90, [1, 0, 0])
          CaptureNut($CaptureNutDiameter = $M3_5CaptureNutDiameter, $CaptureNutHeight   = 3, $Length = 15, $BoreDiameter = 3.2, $BoreLength = 20, $BoreOffset = 6);//M3 nut
      rotate(120, [0, 0, 1])
        translate([0, 7, 0])
          rotate(-90, [1, 0, 0])
            CaptureNut($CaptureNutDiameter = $M3_5CaptureNutDiameter, $CaptureNutHeight   = 3, $Length = 15, $BoreDiameter = 3.2, $BoreLength = 20, $BoreOffset = 6);//M3 nut
