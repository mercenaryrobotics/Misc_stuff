...
module motor(model=Nema23, size=NemaMedium, dualAxis=false, pos=[0,0,0], orientation = [0,0,0], ShowMountShafts = true) {
  ...
  color(stepperBlack){
    translate(pos) rotate(orientation) {
      translate([-mid, -mid, 0]) 
        ...
        if (ShowMountShafts == true)
          translate([-mid, -mid, 0])
            {
            translate([mid+holeDist,mid+holeDist,-20*mm]) cylinder(h=30*mm, r=holeRadius);
            translate([mid-holeDist,mid+holeDist,-20*mm]) cylinder(h=30*mm, r=holeRadius);
            translate([mid+holeDist,mid-holeDist,-20*mm]) cylinder(h=20*mm, r=holeRadius);
            translate([mid-holeDist,mid-holeDist,-20*mm]) cylinder(h=30*mm, r=holeRadius);
          } 
